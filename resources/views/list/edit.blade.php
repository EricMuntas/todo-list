    ...
    <main class="scroll-custom w-full md:w-4/6" id="main">
        <div class="content-frame">
            <div class="list-item-border">
                <h1 class="title-label">Editing "{{$list->title}}"</h1>
                ...
                <form action="{{ route('list.update', $list) }}" method="POST" class="space-y-6 mt-6">
                    ...
                    {{-- Campo de título --}}
                    <div>
                        <label for="title" class="color-label">Title:</label>
                        <input type="text" name="title" id="title" value="{{ old('title', $list->title) }}"
                            placeholder="Title..."
                    ...
                    <div>
                        <label for="description" class="color-label">Description:</label>
                    ...
                    <div class="flex gap-6">
                        <div>
                            <label for="category" class="color-label">Category:</label>
                            <select name="category" id="">
                                <option value="home" {{ old('category', $list->category) == 'home' ? 'selected' : '' }}>
                                    Home
                                </option>
                                <option value="work" {{ old('category', $list->category) == 'work' ? 'selected' : '' }}>
                                    Work
                                </option>
                                <option value="social" {{ old('category', $list->category) == 'social' ? 'selected' : '' }}>
                                    Social</option>
                                <option value="others" {{ old('category', $list->category) == 'others' ? 'selected' : '' }}>
                                    Others</option>
                            </select>
                        </div>
                        <div>
                            <label for="priority" class="color-label">Priority:</label>
                            <select name="priority" id="">
                                <option value="normal" {{ old('priority', $list->priority) == 'normal' ? 'selected' : '' }}>
                                    Normal</option>
                                <option value="important"
                                    {{ old('priority', $list->priority) == 'important' ? 'selected' : '' }}>Important
                                </option>
                            </select>
                        </div>
                    </div>
                    <div>
                        <label for="title" class="color-label">Due to:</label>
                        <input type="date" name="due_to" id="due_to" value="{{ old('due_to', $list->due_to) }}"
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        @error('due_to')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
